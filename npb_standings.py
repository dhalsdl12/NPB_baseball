...
def extract_standings(driver, url_c, url_p):
    ...
    for i in range(1, len(pacific_rows)+1):
        try:
            team = driver.find_element(By.XPATH, f'{pacific_prefix}[{i}]/td[2]').text.strip()
            game = driver.find_element(By.XPATH, f'{pacific_prefix}[{i}]/td[3]').text.strip()
            win = driver.find_element(By.XPATH, f'{pacific_prefix}[{i}]/td[4]').text.strip()
            lose = driver.find_element(By.XPATH, f'{pacific_prefix}[{i}]/td[5]').text.strip()
            tie = driver.find_element(By.XPATH, f'{pacific_prefix}[{i}]/td[6]').text.strip()
            pct = driver.find_element(By.XPATH, f'{pacific_prefix}[{i}]/td[7]').text.strip()
            gb = driver.find_element(By.XPATH, f'{pacific_prefix}[{i}]/td[8]').text.strip()
